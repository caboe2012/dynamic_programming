''' AlgoriComputing the Global Alingment of X = ACC and Y = TTTACACGG
...
Line 1 = alphabet to use
Line 2 = gene sequence x
Line 3 = gene sequence y
Line 4 = match score, mismatch score, insertion score, deletion score
...
Repeat lines 1 -4 according to the number of analyses that will be run.
'''
letters = set(["A","C","T","G"])
diag_score1 = 10
off_diag_score1 = 6
...
def compute_alignment_matrix(seq_x, seq_y, scoring_matrix, global_flag):
	...
	that will be used for to compute an optimal Global Alignment.  The flag
...
X = "ACC"
Y = "TTTACACGG"
_S_ = compute_alignment_matrix(X,Y,_M_,False)
#for row in _S_:
#	print row
...
ans = compute_local_alignment(X,Y,_M_,_S_)
for each in ans:
	print each
'''
...
#	outFile = sys.argv[2]
	...
	for entry in range(entries):
		...
		_M_ = build_scoring_matrix(_alpha, _match, _mismatch, _insert, _delete)
